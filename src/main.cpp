...
int main(int argc, char **argv)
{
    ...
    // Set up barrier
    Barrier *barrier = sequential ? NULL : alloc_barrier(opts.spin, opts.n_threads);

    // Setup args & read input data
    prefix_scan_args_t *ps_args = alloc_args(opts.n_threads);
    int n_vals;
    int *input_vals, *output_vals;
    read_file(&opts, &n_vals, &input_vals, &output_vals);
    
    // Set up tree
    int *tree = sequential ? NULL : alloc_tree(n_vals);

    //"op" is the operator you have to use, but you can use "add" to test
    int (*scan_operator)(int, int, int);
    scan_operator = op;

    fill_args(ps_args, opts.n_threads, n_vals, input_vals, output_vals, tree,
        barrier, scan_operator, opts.n_loops);
    ...
    if (sequential)  {
        ...
        start_threads(threads, opts.n_threads, ps_args, compute_prefix_scan);
